...
publish: convert-rst
	...
	twine upload dist/*
test:
	python -m pytest blotch_tests --capture=no
start-test-server:
	python -m blotch.server --log-dir /tmp/ml-logger-debug
test-with-server:
	python -m pytest blotch_tests --capture=no --log-dir http://0.0.0.0:8081
deploy-vis-app:
